    ...
    public static void main(String[] args) {
        ...
        while (!blockingQueue.isEmpty()) {
            try {
                ...
                System.out.println("Waiting time is " + TimeUnit.SECONDS.convert(delayedWorker.getDuration(), TimeUnit.MILLISECONDS));
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
...
class DelayedWorker implements Delayed {
    ...
    public int compareTo(Delayed otherDelayed) {
        return Long.compare(this.expiredTime, ((DelayedWorker) otherDelayed).getExpiredTime());

    }

    @Override
    public long getDelay(TimeUnit timeUnit) {
        return timeUnit.convert(expiredTime - System.currentTimeMillis(), TimeUnit.MILLISECONDS);
    }
